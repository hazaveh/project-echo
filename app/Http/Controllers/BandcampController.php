...
class BandcampController extends Controller
{
    ...
    {
        return Cache::remember('bandcamp-albums-'.md5($search), 360, function () use ($search, $action) {
    ...
    {
        return Cache::remember('bandcamp-artist-albums-'.md5($username), 360, function () use ($username, $action) {
    ...
    public function albumDetails(string $username, string $albumSlug, FetchAlbumDetailsAction $action): array
    {
        ...
        return Cache::remember($cacheKey, 360, function () use ($username, $albumSlug, $action) {
            try {
    ...
    public function albumDetailsByType(string $username, string $releaseType, string $albumSlug, FetchAlbumDetailsAction $action): array
    {
        ...
        return Cache::remember($cacheKey, 360, function () use ($username, $releaseType, $albumSlug, $action) {
            try {
